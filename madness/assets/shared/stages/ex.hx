...
var contrastShader = createRuntimeShader('contrast');
var glowShader;
...
function onCreatePost()
{
	...
	insert(members.indexOf(uiGroup), exUI);
	exUI.cameras = [camHUD];
	...
	iconP1.setPosition(850, ClientPrefs.data.downScroll ? -10 : 600);
	iconP2.setPosition(280, ClientPrefs.data.downScroll ? -15 : 585);
	...
	{
		glitch = createRuntimeShader("glitch");
		...
		glowShader = createRuntimeShader("wave");
...
function gremlinHit()
{
	...
	var prevPos = exBar.getPosition();
	// whoever looks at this forgive me
	new FlxTimer().start(1.1, function()
	{
		health -= 1;
		FlxTween.tween(exBar, {x: exBar.x + 50}, 0.2, {
...
function onUpdatePost(elapsed)
{
	...
	{
		opponentStrums.members[i].x = baseOppPos[i][0] + FlxG.random.int(-3, 3);
		opponentStrums.members[i].y = baseOppPos[i][1] + FlxG.random.int(-3, 3);
	}
...
function onBeatHit()
{
	...
	if (ClientPrefs.data.unfairGimmicks)
	{
		...
		if (health > gremlinHealthCap && FlxG.random.bool(10))
		{
			gremlinHit();
		}
	}
		...
		case '':
			switch (v1)
			{
				...
				case "Darken Stage":
					...
					darkened = true;
					laser.visible = true;
					camGame.flash(FlxColor.WHITE, 0.6);
				...
				case "End Shit":
					...
					{
						camGame.flash(FlxColor.BLACK, 1000);
					});
...
{
	// cameraZoomMult = mustHitSection ? 1 : 1.1;
}
